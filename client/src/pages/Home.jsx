...
//   );
// }
const API = "https://tech-rnhk.onrender.com"; // ✅ tera backend

const handleBook = async () => {
  if (!form.name || !form.phone) {
    setMsg({ type:'error', text:'Name aur phone number dalna zaroori hai!' });
    return;
  }
  ...
  try {
    ...
    // ✅ SAFE JSON parse (error fix)
    let data;
    try {
      data = await res.json();
    } catch {
      throw new Error("Server response invalid (backend error)");
    }

    if (res.ok && data.success) {
      setMsg({
        type:'success',
        text:`Demo booked, ${form.name}! Team jaldi contact karegi.`
      });
    ...
    setMsg({
      type:'error',
      text: e.message || 'Server error, dobara try karo.'
    });
  } finally {
    setSub(false);
  }
